      ...
      <b-container fluid>
        <b-row>
          <b-col class="clearfix" col lg="5" md="6" xs="12">
            ...
            <router-link :to="{ name: 'helper-affiliate-list' }" class="float-right btn btn-outline-success">Back to link</router-link>
          </b-col>
        </b-row>
      </b-container>
    </template>
        ...
        <b-col col lg="5" md="6" xs="12">
          <validation-observer ref="affiliateForm" #default="{ invalid }">
            <b-form class="auth-login-form mt-2" @submit.prevent="submit">
              ...
              <b-form-group>
                <h3>Referral Link</h3>
                <router-link :to="`${APP_URL}/brokerage/`">{{ `${APP_URL}/brokerage/` }}</router-link>
              </b-form-group>
              ...
              <b-form-group label="Accent Color">
                <validation-provider #default="{ errors }" name="Accent Color" vid="accent_color">
                  <b-form-input v-model="affiliate.accent_color" :state="errors.length > 0 ? false : null" name="accent_color" />
                  <small class="text-danger">{{ errors[0] }}</small>
                </validation-provider>
              </b-form-group>
              ...
              <b-form-group label="Website">
                <validation-provider #default="{ errors }" name="Website" vid="website">
                  <b-form-input v-model="affiliate.website" :state="errors.length > 0 ? false : null" name="website" />
                  <small class="text-danger">{{ errors[0] }}</small>
                </validation-provider>
              </b-form-group>
              ...
              <b-form-group label="PoA Verbiage">
                <validation-provider #default="{ errors }" name="PoA Verbiage" vid="poaVerbiage">
                  <b-form-textarea v-model="affiliate.poaVerbiage" :state="errors.length > 0 ? false : null" name="poaVerbiage" rows="3" />
                  <small class="text-danger">{{ errors[0] }}</small>
                </validation-provider>
              </b-form-group>
              ...
              <b-form-group label="PoA Company Info">
                <validation-provider #default="{ errors }" name="PoA Company Info" vid="poaCompanyInfo">
                  <b-form-input v-model="affiliate.poaCompanyInfo" :state="errors.length > 0 ? false : null" name="poaCompanyInfo" />
                  <small class="text-danger">{{ errors[0] }}</small>
                </validation-provider>
              </b-form-group>
              ...
              <b-form-group label="PoA Thank you URL">
                <validation-provider #default="{ errors }" name="PoA Thank you URL" vid="poaThankyouUrl">
                  <b-form-input v-model="affiliate.poaThankyouUrl" :state="errors.length > 0 ? false : null" name="poaThankyouUrl" />
                  <small class="text-danger">{{ errors[0] }}</small>
                </validation-provider>
              </b-form-group>
...
export default {
  ...
  methods: {
    ...
    ...mapActions('affiliate', { createAffiliate: 'create' }),
    async submit() {
      try {
        const t = await this.createAffiliate(this.affiliate)
        this.$toast({
