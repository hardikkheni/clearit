            ...
            <b-form class="mt-2" @submit.prevent="submit">
              ...
              <b-form-group label="Username/Login">
                <validation-provider #default="{ errors }" name="Username" vid="login" rules="required">
                  <b-form-input v-model="agent.login" :state="errors.length > 0 ? false : null" name="Username" placeholder="john" />
              ...
              <b-form-group label="Firstname" label-for="firstname">
                <validation-provider #default="{ errors }" name="Firstname" vid="firstname" rules="required">
                  <b-form-input v-model="agent.firstname" :state="errors.length > 0 ? false : null" name="Firstname" placeholder="john" />
                  <small class="text-danger">{{ errors[0] }}</small>
                </validation-provider>
              </b-form-group>
...
export default {
  ...
  data() {
    return {
      ...
      countries: [],
      // rules: [
      //   { text: 'Account Verification', value: 0 },
      //   { text: 'Commodity & Doc Review', value: 1 },
      //   { text: ' ISF Coordinator', value: 2 },
      //   { text: 'Prekey & Billing', value: 3 },
      //   { text: 'Arrival Notice Review and Release Processing', value: 4 },
      //   { text: 'Delivery Coordinator', value: 5 },
      //   { text: 'Customer Service', value: 6 },
      // ],
    }
  },
  computed: {
  ...
  methods: {
    ...
    ...mapActions('agent', { createAgent: 'create' }),
    ...mapMutations('agent', { setState: 'setState' }),
    async submit() {
      try {
        const t = await this.createAgent(this.agent)
        this.$toast({
