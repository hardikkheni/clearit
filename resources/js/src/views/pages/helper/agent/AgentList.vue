      ...
      <b-container fluid>
        <b-row>
          <b-col sm="12" md="4" align-self="start">
            <span class="h2 align-middle mr-1">Agents</span>
            <router-link :to="{ name: 'create-helper-agent' }" class="d-inline-block btn btn-outline-success">
    ...
    <b-container fluid>
      <data-table ref="agentTable" :filter="search" :extra="extra" :provider="dataTable" :columns="columns" @row-clicked="rowClicked">
        ...
        <template #cell(city)="{ item }">
          {{ `${[...(item.city ? [item.city] : []), ...(item.state ? [item.state] : [])].join(', ')}` }}
        </template>
...
export default {
  ...
  data() {
    return {
      statusOptions: [
        { text: 'Active Agents Only', value: true },
        { text: 'All Agents', value: null },
      ],
      columns: [
        { key: 'login', label: 'USERNAME', sortable: true },
        { key: 'firstname', label: 'NAME', sortable: true },
        { key: 'city', label: 'CITY / STATE', sortable: true },
  ...
  methods: {
    ...mapActions('agent', { dataTable: 'dataTable' }),
    triggerSearch() {
