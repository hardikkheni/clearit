      ...
      <b-container fluid>
        <b-row>
          <b-col class="clearfix" col lg="5" md="6" xs="12">
            <h2 class="float-left">Add Contact</h2>
            <router-link
              :to="{
                name: 'edit-helper-freight-forwarder',
                params: { id: $route.params.ffId },
              }"
              class="float-right btn btn-outline-success"
            >
              Back to list
            </router-link>
          </b-col>
...
import { BFormGroup, BFormInput, BForm, BButton, BCard, BRow, BCol, BFormCheckbox, BContainer } from 'bootstrap-vue'
...
export default {
  ...
  beforeDestroy() {
    this.setState({ key: 'freightContact', value: { ...this.defaultFreightContact } })
  },
  methods: {
    ...mapActions('freight-forwarder', { createFreigntContact: 'createContact', findFreightForwarderById: 'findById' }),
    ...mapMutations('freight-forwarder', { setState: 'setState' }),
    async submit() {
      try {
        const t = await this.createFreigntContact({
          ffId: this.$route.params.ffId,
          payload: this.freightContact,
        })
        this.$toast({
